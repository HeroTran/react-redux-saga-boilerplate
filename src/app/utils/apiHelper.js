...
import { STATUS_RESPONSES } from '../functionals/Common/constants';
...
export function postUploadFile(request, progressCb, cancelToken) {
  let req;
  if (request.uuid) {
    req = {
...
function handleErrorUnauthorized(error) {
  ...
  if (status === STATUS_RESPONSES.UNAUTHORIZED) {
